...
<%
    ...
    try {
        ...
        if (rs.next()) {
            ...
            int i = st1.executeUpdate("INSERT into request(filename, time, uid, uname, status, fid, doid, umail, dkey) values('" + fname + "','" + time + "','" + uid + "','" + uname + "','waiting','" + fid + "', '" + doid+ "','" + umail+ "','"+dkey+"')");
            if (i != 0) {
                response.sendRedirect("Search_file.jsp?Requestsent");
            }
        } else {
